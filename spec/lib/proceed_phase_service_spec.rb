...
RSpec.describe ProceedPhaseService, type: :service do
  ...
  describe '#call' do
    ...
    context "ロビーから廃卓へ" do
      ...
      example "更新後の卓のステータスが DISCARDED であること" do
        travel_to('2019-05-12 07:00') do
          expect(table.status_text).to eq Table.status_text(code: Table::DISCARDED)
        end
      end
    end
    ...
    context "ロビーから開始へ" do
      ...
      example "卓のステータスが STARTED であること" do
        travel_to('2019-05-12 07:00') do
          expect(table.status_text).to eq Table.status_text(code: Table::STARTED)
        end
      end

      example "卓のターンが 1 年目の春外交フェイズであること" do
        travel_to('2019-05-12 07:00') do
    ...
    context "春外交から秋外交へ" do
      before :example do
        ...
        @table.current_turn.orders << MoveOrder.new(power: power_a, unit: unit_e_lon, dest: 'nth')
        # 保存
        @table.save!
      end
      ...
      example "更新前には F lon-nth の仮想命令が登録されていること" do
        travel_to('2019-05-12 06:50') do
          orders = table.current_turn.orders.where(phase: Const.phases.spr_1st)
      ...
      example "卓のターンが 1 年目の秋外交フェイズであること" do
        travel_to('2019-05-12 07:00') do
          expect(table.turn).to eq 1
          expect(table.phase).to eq Const.phases.fal_1st
        end
      end
      ...
      example "1 年目の春外交フェイズに 22 のユニットが生成されていること" do
        travel_to('2019-05-12 07:00') do
          ...
          units = turn.units.where(phase: Const.phases.spr_1st)
      ...
      example "1 年目の春撤退フェイズに 22 のユニットが生成されていること" do
        travel_to('2019-05-12 07:00') do
          turn = table.turns.find_by(number: 1)
          units = turn.units.where(phase: Const.phases.spr_2nd)
          expect(units.size).to eq 22
        end
      end
    end
    ...
    context "春外交から春撤退へ" do
      before :example do
        ...
        order = ListPossibleOrdersService.call(turn: turn, power: power_e, unit: unit_e).detect{|o| o.hold?}
        order.keepout = 'eng'
      ...
      example "卓のターンが 1 年目の春撤退フェイズであること" do
        travel_to('2019-05-12 07:00') do
          expect(table.turn).to eq 1
          expect(table.phase).to eq Const.phases.spr_2nd
        end
      end
      ...
      example "1 年目の春撤退フェイズに F lon の解散命令が生成されていること" do
        travel_to('2019-05-12 07:00') do
          turn = table.turns.find_by(number: 1)
          orders = turn.orders.where(phase: Const.phases.spr_2nd)
          expect(orders.size).to eq 1
    ...
    context "秋撤退から制覇終了へ" do
      before :example do
        ...
        @table = @table.start
        # 1901 年秋撤退フェイズに進む
        @table.phase = Const.phases.fal_2nd
      ...
      example "卓のステータスが制覇終了であること" do
        travel_to('2019-05-12 07:00') do
          expect(table.status_text).to eq Table.status_text(code: Table::SOLO)
        end
      end
    ...
    context "秋撤退から翌春外交へ" do
      before :example do
        ...
        # tun にイギリス海軍設置
        power_e = @table.powers.find_by(symbol: 'e')
        @table.current_turn.units.create(type: Fleet.to_s, power: power_e, province: 'tun', phase: @table.phase)
        # 更新期限調整
        @table.period = '2019-05-12 07:00'
        @table.save!
      end
    ...
    context "秋撤退から秋調整へ：増設可能" do
      before :example do
        ...
        @table.current_turn.units.create(type: Fleet.to_s, power: power_e, province: 'tun', phase: @table.phase)
        # lon からユニット除去
        @table.current_turn.units.where(phase: @table.phase).find_by(province: 'lon').delete
      ...
      example "卓のターンが 2 年目の秋調整フェイズであること" do
        travel_to('2019-05-12 07:00') do
          expect(table.turn).to eq 1
          expect(table.phase).to eq Const.phases.fal_3rd
        end
      end

      example "イギリスの領地が 4 であること" do
        travel_to('2019-05-12 07:00') do
          supplycenters = table.current_turn.provinces.where(power: 'e').where(supplycenter: true)
