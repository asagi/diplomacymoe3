class SearchReachableCoastalsService
  ...
  def call
    ...
    marked << @unit.province if MapUtil.provinces[@unit.province]["type"] == Water.to_s
    coastals = []
    marked.each do |water|
      MapUtil.adjacents[water].each do |k, v|
        next if k[0, 3] == @unit.province
        next unless MapUtil.provinces[k]["type"] == Coastal.to_s
        coastals << k
      end
    end
    coastals -= MapUtil.adjacents[@unit.province].select { |k, v| v["army"] }.keys
